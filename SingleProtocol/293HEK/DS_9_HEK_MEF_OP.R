...
setwd("~/R/Projects/Seurat/DropSeq/DS_9_HEK_MEF")
set.seed(42)

# Determine which cells are Human (293HEK) from the mixed species analysis
human <- read.table("server/HEK_MEF_OP_DGE_Human_Summary.txt", header=T) ##
human <- human %>% mutate(sample="human")
mouse <- read.table("server/HEK_MEF_OP_DGE_Mouse_Summary.txt", header=T) ##
...
DGE.data <- read.table(file = "server/HEK_MEF_OP_DGE.txt", header = TRUE, row.names = 1)
...
DGE <- CreateSeuratObject(raw.data = DGE.data[,intersect(humanCells, colnames(DGE.data))], 
                          min.cells = 3, min.genes = 200, project = "DropSeq_293HEK_DS9_OP")
...
DGE <- FilterCells(object = DGE, subset.names = c("nGene", "percent.mito"), 
                   low.thresholds = c(200, -Inf), high.thresholds = c(4000, 0.06))
...
DropSeq_293HEK_DS9_OP <- DGE
saveRDS(DropSeq_293HEK_DS9_OP, file='~/R/Projects/Seurat/Robj/NotScaled/DropSeq/DropSeq_293HEK_DS9_OP.Robj')
...
a <- read.table("server/HEK_MEF_OP_cell_readcounts.txt", 
...
write.table(x=topBarcodes, file = "DS9_HEK_MEF_OP_top_barcodes.txt", 
...
p <- ggplot(combined, aes (x,y , colour=(Species) )) + 
  ...
  ggtitle('DS_9_HEK_MEF_OP',subtitle =  'Mixed-Species plot with top 400 cells') + 
...
#save human-only DS_9
DropSeq_HEK_DS9_OP <- DGE
save(DropSeq_HEK_DS9_OP, file = '~/R/Projects/Seurat/Robj/DropSeq/DropSeq_HEK_DS9_OP_v2.1.Robj')
...
DGE <- FindClusters(object = DGE, reduction.type = "pca", dims.use = 1:40, 
                    resolution = 0.6, print.output = 0, save.SNN = TRUE, force.recalc = TRUE)
PrintFindClustersParams(object = DGE)

#### DS9_OP both species: 16 pca dims, 30 perpl, 200-5000 genes, 0.05 mt
#### DS9_OP human only: 40, same as above
# Note: many human cells are dropped @ 0.05mt
DGE <- RunTSNE(object = DGE, dims.use = 1:40, do.fast = TRUE, perplexity = 30)
t <- TSNEPlot(object = DGE, do.return = TRUE)
t + ggtitle('DS9 HEK-OP tSNE v2.1')
...
t <- TSNEPlot(object = DGE, do.return=TRUE)
t + ggtitle('DS9 HEK-MEF-OP tSNE v2.1 Species Analysis')

#################################################################################
# Save object for future cross-analyses
DS9_HEK_OP <- DGE
save(DS9_HEK_OP, file='~/R/Projects/Seurat/DropSeq/Robj/DS9_HEK_OP_v2.1.Robj')
